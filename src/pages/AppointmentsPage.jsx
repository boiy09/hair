...
export default function AppointmentsPage() {
  ...
  async function handleStatusChange(appt, newStatus) {
    if (newStatus === 'noshow') {
      const ok = await confirm({
        title: '노쇼 처리',
        message: `"${appt.customerName}" 고객을 노쇼 처리하시겠어요? 노쇼 카운트가 1 올라갑니다.`,
        confirmLabel: '노쇼 처리',
        danger: true,
      });
      if (!ok) return;
  ...
  async function handleDelete(appt) {
    const ok = await confirm({
      title: '예약 삭제',
      message: '이 예약을 삭제하시겠어요? 복구 불가합니다.',
      confirmLabel: '삭제',
      danger: true,
    });
    if (!ok) return;
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          {isToday && <div style={{ fontSize: 12, color: 'var(--gold)', marginTop: 2 }}>오늘</div>}
        </div>
        <button className="btn btn-ghost" style={{ minHeight: 40, padding: '0 12px' }} onClick={() => changeDate(1)}>
      ...
      <div style={{ padding: '10px 16px 6px', display: 'flex', gap: 6, overflowX: 'auto' }}>
        {[['all', '전체'], ['pending', '대기'], ['confirmed', '확정'], ['completed', '완료'], ['cancelled', '취소'], ['noshow', '노쇼']].map(([val, label]) => (
          <button
            key={val}
            className={`chip ${filterStatus === val ? 'active' : ''}`}
      ...
      <div className="page-content">
        ...
        <div className="section-header">
          <div className="section-title">
            {isToday ? '오늘' : formatDate(selectedDate)} 예약
          </div>
        ...
        {todayAppts.length === 0 ? (
          ...
          <div style={{ background: 'var(--bg-card)', margin: '0 16px', borderRadius: 'var(--radius)' }}>
            {todayAppts.map(appt => (
              <AppointmentItem
                key={appt.id}
                appt={appt}
          ...
          <>
            ...
            <div style={{ background: 'var(--bg-card)', margin: '0 16px', borderRadius: 'var(--radius)' }}>
              ...
              {upcomingAppts.length > 5 && (
                <div style={{ padding: '12px 16px', textAlign: 'center', fontSize: 14, color: 'var(--text-secondary)' }}>
    ...
    <div style={{ borderBottom: '1px solid var(--border)' }}>
      ...
      {expanded && (
        <div style={{ padding: '0 16px 14px', display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          ...
          <button
            className="btn"
            style={{ fontSize: 13, minHeight: 38, padding: '0 14px', background: 'var(--red-dim)', color: 'var(--red)' }}
          ...
          {appt.request && (
            <div style={{ width: '100%', fontSize: 13, color: 'var(--text-secondary)' }}>
              요청: {appt.request}
            </div>
          )}
        </div>
...
function AppointmentForm({ onClose, onSave, initialDate }) {
  ...
  function toggleMenu(menu) {
    ...
    if (menus.includes(menu.name)) {
      setForm(f => ({ ...f, menus: f.menus.filter(m => m !== menu.name) }));
    } else {
      setForm(f => ({ ...f, menus: [...f.menus, menu.name] }));
    }
  }
        ...
        <div className="form-group">
          ...
          <input
            ...
            onChange={e => {
              ...
              setForm(f => ({ ...f, customerName: e.target.value, customerId: '', customerPhone: '' }));
            }}
          />
          {customerSearch && filteredCustomers.length > 0 && (
            <div style={{ background: 'var(--bg-elevated)', borderRadius: 'var(--radius-sm)', marginTop: 4 }}>
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
            {menus.map(menu => (
              <button
                key={menu.id}
                className={`chip ${form.menus.includes(menu.name) ? 'active' : ''}`}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20, padding: '12px 0' }}>
          ...
          <div
            ...
            style={{
              ...
              background: form.alertEnabled ? 'var(--gold)' : 'var(--bg-elevated)',
