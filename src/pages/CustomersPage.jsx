...
import { Plus, Search, X, SlidersHorizontal, ChevronRight } from 'lucide-react';
import Header from '../components/Header';
import {
  getCustomers, addCustomer, updateCustomer, deleteCustomer,
  getGrade, getGradeBadgeClass, getCustomerById
} from '../store';
import {
  formatPhone, formatDate, getDaysAgo, getInitials, matchesSearch, validatePhone, today
} from '../utils';
...
const FILTER_OPTIONS = {
  ...
  lastVisit: [
    ...
    { label: '3개월 이내', value: '90' },{ label: '3개월 초과', value: '90+' },
  ],
  grade: [
      ...
      <div style={{ padding: '12px 16px 4px', display: 'flex', gap: 8 }}>
        <div style={{ flex: 1, position: 'relative' }}>
          ...
          <input className="input" style={{ paddingLeft: 40, minHeight: 44 }}
            placeholder="이름, 연락처 뒷 4자리, 초성 검색"
        ...
        <button className={`btn ${showFilter ? 'btn-gold' : 'btn-ghost'}`} style={{ minHeight: 44, padding: '0 14px' }} onClick={() => setShowFilter(!showFilter)}>
...
function CustomerForm({ onClose, onSave, existingCustomers, initial }) {
  const [form, setForm] = useState({
    ...
    gender: initial?.gender || '', source: initial?.source || '', allergy: initial?.allergy || '',
    memo: initial?.memo || '', tags: initial?.tags || [], blacklist: initial?.blacklist || false,
  });
  ...
  function addTag() {
    ...
    if (!t) return;
    if (form.tags.length >= 5) { toast('태그는 최대 5개까지 등록 가능합니다', 'error'); return; }
