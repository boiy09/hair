...
export default function SalesPage() {
  ...
  const barData = useMemo(() => {
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      if (m < 0) { m += 12; y--; }
      const ms = getMonthlySales(y, m);
      data.push({ label: `${m + 1}월`, total: ms.reduce((sum, s) => sum + (s.total || 0), 0) });
    }
    return data;
  }, [sales]);
  ...
  const menuBreakdown = useMemo(() => {
    const map = {};
    monthlySales.forEach(s => (s.items || []).forEach(item => { if (!map[item.name]) map[item.name] = 0; map[item.name] += item.price || 0; }));
    return Object.entries(map).sort((a, b) => b[1] - a[1]);
  }, [monthlySales]);

  const paymentBreakdown = useMemo(() => {
    const map = {};
    monthlySales.forEach(s => { const m = s.paymentMethod || '현금'; if (!map[m]) map[m] = 0; map[m] += s.total || 0; });
    return Object.entries(map).sort((a, b) => b[1] - a[1]);
  }, [monthlySales]);

  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Header title="매출 관리" rightAction={
      ...
      {/* Tab selector */}
      <div style={{ display: 'flex', margin: '12px 16px 4px', background: 'var(--surface-low)', borderRadius: 9999, padding: 4 }}>
        ...
        {tab === 'list' ? (
          <>
            <div className="card" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                ...
                <div style={{ fontSize: 22, fontWeight: 700, color: 'var(--primary-dark)', marginTop: 2 }}>{formatMoney(todayTotal)}</div>
              </div>
            ...
            {sales.length === 0 ? (
              <div className="empty-state">
                ...
                <button className="btn btn-gold" onClick={() => setShowForm(true)}><Plus size={16} /> 첫 매출 기록</button>
              </div>
            ) : (
              <div>
                {groupByDate(sales).map(({ date, items }) => (
                  <div key={date}>
                    <div style={{ padding: '10px 16px 4px', display: 'flex', justifyContent: 'space-between' }}>
                      <span style={{ fontSize: 14, fontWeight: 600, color: 'var(--text-secondary)' }}>{formatDate(date)}</span>
                      <span style={{ fontSize: 14, color: 'var(--primary-dark)', fontWeight: 600 }}>{formatMoney(items.reduce((sum, s) => sum + (s.total || 0), 0))}</span>
                    </div>
                    <div style={{ padding: '0 16px 8px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {items.map(sale => <SaleItem key={sale.id} sale={sale} />)}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 16px 12px' }}>
              <button className="btn btn-ghost" style={{ minHeight: 40, padding: '0 12px' }} onClick={() => changeMonth(-1)}><ChevronLeft size={20} /></button>
              <div style={{ fontWeight: 700, fontSize: 18 }}>{viewYear}년 {MONTH_NAMES[viewMonth]}</div>
              <button className="btn btn-ghost" style={{ minHeight: 40, padding: '0 12px' }} onClick={() => changeMonth(1)}><ChevronRight size={20} /></button>
            </div>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 120 }}>
                {barData.map((d, i) => (
                  <div key={i} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                    <div style={{ fontSize: 10, color: 'var(--text-tertiary)' }}>{d.total > 0 ? `${Math.round(d.total / 10000)}만` : ''}</div>
                    <div style={{
                      width: '100%',
              ...
              <div className="card">
                ...
                {menuBreakdown.map(([name, amount]) => (
                  <div key={name} style={{ marginBottom: 10 }}>
                    ...
                    <div style={{ height: 6, background: 'var(--surface-high)', borderRadius: 3 }}>
                      <div style={{ height: '100%', width: `${(amount / (menuBreakdown[0][1] || 1)) * 100}%`, background: 'var(--primary)', borderRadius: 3 }} />
                    </div>
                  </div>
                ))}
              </div>
            ...
            {paymentBreakdown.length > 0 && (
              <div className="card">
                ...
                {paymentBreakdown.map(([method, amount], i) => (
                  <div key={method} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 0', borderTop: i > 0 ? '1px solid var(--outline)' : 'none' }}>
                    <span style={{ fontSize: 14 }}>{method}</span>
                    <div style={{ textAlign: 'right' }}>
                      <div style={{ fontSize: 14, fontWeight: 600 }}>{formatMoney(amount)}</div>
                      <div style={{ fontSize: 12, color: 'var(--text-tertiary)' }}>{monthTotal > 0 ? `${Math.round((amount / monthTotal) * 100)}%` : ''}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      {showForm && (
        <SaleForm onClose={() => setShowForm(false)} onSave={() => { refresh(); setShowForm(false); toast('매출이 기록되었어요!', 'success'); }} />
      )}
    </div>
  );
}
...
function SaleItem({ sale }) {
  return (
    <div style={{ background: 'var(--card)', borderRadius: 'var(--radius)', padding: '14px 16px', display: 'flex', alignItems: 'center', boxShadow: '0 1px 8px rgba(26,27,31,0.05)' }}>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 600 }}>{sale.customerName || '고객'}</div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{(sale.items || []).map(i => i.name).join(' + ') || '시술'}</div>
        <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 2 }}>
          {sale.paymentMethod || '현금'}{sale.discount > 0 ? ` · 할인 ${(sale.discount || 0).toLocaleString()}원` : ''}
        </div>
      </div>
      <div style={{ textAlign: 'right', flexShrink: 0 }}>
        <div style={{ fontWeight: 700, color: 'var(--primary-dark)' }}>{formatMoney(sale.total || 0)}</div>
        {sale.discount > 0 && <div style={{ fontSize: 12, color: 'var(--text-tertiary)', textDecoration: 'line-through' }}>{formatMoney(sale.subtotal || 0)}</div>}
      </div>
    </div>
  );
}
...
function groupByDate(sales) {
  const map = {};
  sales.forEach(s => { const d = s.date || today(); if (!map[d]) map[d] = []; map[d].push(s); });
  return Object.entries(map).sort(([a], [b]) => b.localeCompare(a)).map(([date, items]) => ({ date, items }));
}
