...
export default function SaleForm({ customerId, customerName, onClose, onSave, initial }) {
  ...
  const [form, setForm] = useState({
    ...
    paymentMethod: initial?.paymentMethod || '현금',
  ...
  function toggleMenu(menu) {
    ...
    if (exists) setForm(f => ({ ...f, items: f.items.filter(i => i.id !== menu.id) }));
    else setForm(f => ({ ...f, items: [...f.items, { id: menu.id, name: menu.name, price: menu.price }] }));
  }

  function updateItemPrice(id, price) {
    setForm(f => ({ ...f, items: f.items.map(i => i.id === id ? { ...i, price: Number(price) || 0 } : i) }));
  }
  ...
  function handlePhotos(e) {
    ...
    if (form.photos.length + files.length > 10) { toast('기록 1건당 최대 10장까지 등록 가능합니다', 'error'); return; }
    const oversized = files.filter(f => f.size > 10 * 1024 * 1024);
    if (oversized.length > 0) { toast('10MB 이하 파일만 가능합니다', 'error'); return; }
  ...
  function handleSave() {
    ...
    if (total > 1000000 && !window.confirm(`${formatMoney(total)}이 맞나요?`)) return;
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-sheet" onClick={e => e.stopPropagation()}>
        ...
        {customerName && (
          <div style={{ marginBottom: 16, padding: '12px 14px', background: 'var(--primary-container)', borderRadius: 'var(--radius-sm)', fontSize: 14, color: 'var(--primary-dark)' }}>
        ...
        <div className="form-group">
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 8 }}>
            {menus.map(menu => {
              ...
              return (
                <button key={menu.id} className={`chip ${selected ? 'active' : ''}`} onClick={() => toggleMenu(menu)}>
                  {menu.name}
                </button>
              );
            })}
          ...
          {form.items.length > 0 && (
            <div style={{ background: 'var(--surface-low)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
              {form.items.map(item => (
                <div key={item.id} style={{ display: 'flex', alignItems: 'center', padding: '10px 14px', borderBottom: '1px solid var(--outline)' }}>
                  <span style={{ flex: 1, fontSize: 14 }}>{item.name}</span>
                  <input className="input" type="number" value={item.price}
                    onChange={e => updateItemPrice(item.id, e.target.value)}
                    inputMode="numeric" style={{ width: 110, textAlign: 'right', minHeight: 36, padding: '6px 10px' }} />
        ...
        <div className="form-group">
          ...
          <input className="input" placeholder="브랜드명 + 제품명 + 농도"
            value={form.chemical} onChange={e => setForm(f => ({ ...f, chemical: e.target.value }))} />
        </div>

        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="form-group">
            <label className="label">소요 시간 (분)</label>
            <input className="input" type="number" placeholder="예: 90" value={form.duration}
              inputMode="numeric" onChange={e => setForm(f => ({ ...f, duration: e.target.value }))} />
          </div>
          <div className="form-group">
            <label className="label">다음 방문 권장일</label>
            <input className="input" type="date" value={form.nextVisit}
              style={{ colorScheme: 'light' }} onChange={e => setForm(f => ({ ...f, nextVisit: e.target.value }))} />
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          <div className="form-group">
            ...
            <input className="input" type="number" value={form.discount || ''} inputMode="numeric" placeholder="0"
        ...
        {form.discount > 0 && (
          <div className="form-group">
            ...
            <input className="input" placeholder="단골 할인, 이벤트 등"
              value={form.discountReason} onChange={e => setForm(f => ({ ...f, discountReason: e.target.value }))} />
          </div>
        )}

        {form.items.length > 0 && (
          <div style={{ background: 'var(--primary-container)', borderRadius: 'var(--radius-sm)', padding: '14px 16px', marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <span style={{ fontWeight: 600, color: 'var(--primary-dark)' }}>최종 결제액</span>
              <span style={{ fontSize: 20, fontWeight: 700, color: 'var(--primary-dark)' }}>{formatMoney(total)}</span>
            </div>
            {form.discount > 0 && (
              <div style={{ fontSize: 13, color: 'var(--on-secondary-container)', marginTop: 4 }}>
        ...
        <div className="form-group">
              ...
              {form.photos.map((p, i) => (
                <div key={i} style={{ position: 'relative', flexShrink: 0 }}>
                  ...
                  <button
                    ...
                    style={{ position: 'absolute', top: -6, right: -6, width: 20, height: 20, borderRadius: '50%', background: 'var(--error)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <label style={{
            ...
            <input type="file" accept="image/jpeg,image/png,image/heic" multiple style={{ display: 'none' }}
              disabled={form.photos.length >= 10} onChange={handlePhotos} />
          </label>
        </div>

        <div className="form-group">
          <label className="label">특이사항·메모</label>
          <textarea className="input" placeholder="두피 상태, 고객 반응, 다음 시술 주의사항 (최대 400자)"
            value={form.specialNote} maxLength={400} onChange={e => setForm(f => ({ ...f, specialNote: e.target.value }))} />
        </div>
