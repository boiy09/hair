...
export default function SettingsPage() {
  ...
  function addMenu() {
    ...
    const newMenu = { id: Date.now().toString(), name: newMenuName.trim(), price: Number(newMenuPrice) || 0 };
    const updated = [...menus, newMenu];
    setMenus(updated); saveMenus(updated);
  ...
  function exportCSV() {
    ...
    const rows = [
      ...
      ...customers.map(c => [
        c.name, c.phone,
        `${c.visitCount >= 11 ? 'VIP' : c.visitCount >= 6 ? '단골' : c.visitCount >= 2 ? '일반' : '신규'}`,
    ...
    a.href = url; a.download = `miyong_chart_${new Date().toISOString().split('T')[0]}.csv`;
    a.click(); URL.revokeObjectURL(url);
    toast('CSV 파일이 다운로드되었어요', 'success');
  }

  async function clearAllData() {
    const ok = await confirm({ title: '전체 데이터 삭제', message: '모든 고객, 예약, 매출 데이터가 삭제됩니다. 복구 불가합니다. 정말 삭제하시겠어요?', confirmLabel: '전체 삭제', danger: true });
    if (!ok) return;
    localStorage.clear();
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="page-content">
        ...
        <div style={{ textAlign: 'center', padding: '28px 16px 16px' }}>
          <div style={{ width: 64, height: 64, borderRadius: 20, background: 'var(--primary-container)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px', fontSize: 32 }}>✂️</div>
          <div style={{ fontSize: 20, fontWeight: 800, color: 'var(--text)' }}>MIYONG-CHART</div>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 4 }}>미용차트 v1.0</div>
          <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 4 }}>1인 미용사를 위한 고객 관리 시스템</div>
        </div>

        <div className="stats-row">
          <div className="stat-card"><div className="stat-value">{customers.length}</div><div className="stat-label">전체 고객</div></div>
          <div className="stat-card"><div className="stat-value">{appointments.length}</div><div className="stat-label">전체 예약</div></div>
          <div className="stat-card"><div className="stat-value">{sales.length}</div><div className="stat-label">매출 기록</div></div>
        </div>
        ...
        <div className="card">
          {menus.map(menu => (
            <div key={menu.id} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              ...
              <input className="input" type="number" value={menu.price} inputMode="numeric"
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input className="input" style={{ flex: 2 }} placeholder="메뉴 이름"
              value={newMenuName} onChange={e => setNewMenuName(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addMenu()} />
            <input className="input" style={{ flex: 1 }} type="number" inputMode="numeric" placeholder="가격"
              value={newMenuPrice} onChange={e => setNewMenuPrice(e.target.value)} />
        ...
        <div className="card" style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {[['🔐','데이터는 이 기기에 로컬 저장됩니다'],['🛡','개인정보처리방침을 준수합니다'],['💾','CSV 내보내기로 주기적 백업을 권장합니다']].map(([icon, text]) => (
        ...
        <div className="section-header">
          <div className="section-title" style={{ color: 'var(--error)' }}>위험 구역</div>
        </div>
        <div className="card">
