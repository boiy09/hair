...
import { getMenus, saveMenus, getCustomers, getSales, getAppointments, getPresets, savePresets } from '../store';
...
export default function SettingsPage() {
  ...
  // ── Coloring preset management ────────────────────────────────
  function addPreset() {
    const t = newPreset.trim();
    if (!t) { toast('프리셋 내용을 입력해 주세요', 'error'); return; }
    if (presets.includes(t)) { toast('이미 있는 프리셋이에요', 'error'); return; }
    const updated = [...presets, t];
    setPresets(updated);
    savePresets(updated);
    setNewPreset('');
    toast('프리셋이 추가되었어요!', 'success');
  }

  function deletePreset(p) {
    const updated = presets.filter(x => x !== p);
    setPresets(updated);
    savePresets(updated);
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <div className="page-content">
        ...
        {/* Coloring preset management */}
        <div className="section-header">
          <div className="section-title">자주 쓰는 컬러 프리셋</div>
        </div>

        <div className="card">
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 12 }}>
            시술 기록 추가 시 컬러링 빠른 입력에 사용됩니다
          </div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
            {presets.map(p => (
              ...
              >
                ...
                <button
                  ...
                  style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 0, marginLeft: 2, color: 'var(--primary-dark)', display: 'flex' }}
                >
                  <X size={12} />
                </button>
              </span>
            ...
            {presets.length === 0 && (
              <span style={{ fontSize: 13, color: 'var(--text-tertiary)' }}>프리셋이 없어요</span>
            )}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <input
              className="input"
              style={{ flex: 1 }}
              placeholder="예: 8레벨 애쉬블루"
              value={newPreset}
              onChange={e => setNewPreset(e.target.value)}
              onKeyDown={e => e.key === 'Enter' && addPreset()}
            />
            <button className="btn btn-gold" style={{ flexShrink: 0, minHeight: 48 }} onClick={addPreset}>
              <Plus size={18} />
            </button>
          </div>
        </div>
