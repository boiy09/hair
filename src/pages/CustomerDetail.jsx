...
import { Edit2, Trash2, Plus, AlertTriangle, ChevronRight } from 'lucide-react';
...
export default function CustomerDetail({ customerId, onBack, onRefresh }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      <Header title="고객 상세" onBack={onBack} rightAction={
      ...
      <div className="page-content">
        ...
        <div style={{ padding: '20px 16px 12px' }}>
          <div style={{ display: 'flex', gap: 16, alignItems: 'flex-start' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ color: 'var(--text-secondary)', fontSize: 14, marginTop: 4 }}>{customer.phone || '연락처 없음'}</div>
              {customer.birthdate && <div style={{ color: 'var(--text-tertiary)', fontSize: 13 }}>생일 {formatDate(customer.birthdate)}</div>}
              {customer.source && <div style={{ color: 'var(--text-tertiary)', fontSize: 13 }}>유입: {customer.source}</div>}
            </div>
          </div>
        ...
        <div className="stats-row">
          ...
          <div className="stat-card"><div className="stat-value">{customer.noShowCount || 0}</div><div className="stat-label">노쇼</div></div>
          <div className="stat-card">
            <div className="stat-value" style={{ fontSize: 14 }}>{totalSpent > 0 ? formatMoney(totalSpent).replace('원', '') : '0'}</div>
        ...
        <div style={{ padding: '0 16px 12px', display: 'flex', gap: 10 }}>
          ...
          <div className="card" style={{ margin: 0, flex: 1, padding: 12 }}>
            ...
            <div style={{ fontWeight: 600, color: (customer.prepaid || 0) > 0 ? 'var(--green)' : 'var(--text)' }}>{formatMoney(customer.prepaid || 0)}</div>
          </div>
        </div>
        ...
        {upcomingAppts.length > 0 && (
          <div className="card" style={{ background: 'var(--primary-container)' }}>
            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--primary-dark)', marginBottom: 8 }}>📅 예정 예약</div>
            {upcomingAppts.map(a => (
        ...
        <div style={{ padding: '0 16px 12px' }}>
          <button className="btn btn-gold btn-full" onClick={() => setShowSaleForm(true)}><Plus size={18} /> 시술 기록 추가</button>
        </div>

        {/* Tab selector */}
        <div style={{ display: 'flex', margin: '0 16px', background: 'var(--surface-low)', borderRadius: 9999, padding: 4 }}>
          {[['history','시술 이력'],['photos','사진']].map(([key, label]) => (
            <button key={key} onClick={() => setTab(key)} style={{
              ...
              boxShadow: tab === key ? '0 2px 8px rgba(26,27,31,0.08)' : 'none',
          ...
          <div style={{ marginTop: 12 }}>
            {treatments.length === 0 ? (
              <div className="empty-state" style={{ padding: '40px 24px' }}>
                ...
                <button className="btn btn-outline" onClick={() => setShowSaleForm(true)}><Plus size={16} /> 첫 시술 기록</button>
              </div>
            ...
            {treatments.filter(t => t.photos?.length > 0).length === 0 ? (
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
                {treatments.flatMap(t => t.photos || []).map((photo, i) => (
                  <div key={i} style={{ aspectRatio: '1', borderRadius: 'var(--radius-sm)', overflow: 'hidden', background: 'var(--surface-high)' }}>
  ...
  return (
    <div className="card">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
        <div>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{(treatment.items || []).map(i => i.name).join(' + ') || '시술'}</div>
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginTop: 2 }}>{formatDate(treatment.date)}</div>
        </div>
        <div style={{ textAlign: 'right' }}>
          <div style={{ fontWeight: 700, color: 'var(--primary-dark)' }}>{formatMoney(treatment.total || 0)}</div>
          {treatment.discount > 0 && <div style={{ fontSize: 12, color: 'var(--text-tertiary)', textDecoration: 'line-through' }}>{formatMoney(treatment.subtotal || 0)}</div>}
        </div>
      </div>
      {treatment.detail && <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 8 }}>{treatment.detail}</div>}
      {treatment.chemical && <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginBottom: 8 }}>💊 {treatment.chemical}</div>}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
          {treatment.paymentMethod || '현금'} 결제{treatment.duration ? ` · ${treatment.duration}분` : ''}
        </span>
        {treatment.nextVisit && <span style={{ fontSize: 12, color: 'var(--primary-dark)' }}>다음 방문 권장: {formatDate(treatment.nextVisit)}</span>}
      </div>
      {treatment.photos?.length > 0 && (
        <div style={{ display: 'flex', gap: 6, marginTop: 10, overflowX: 'auto' }}>
          {treatment.photos.slice(0, 4).map((p, i) => (
            <img key={i} src={p} alt="" style={{ width: 64, height: 64, borderRadius: 8, objectFit: 'cover', flexShrink: 0 }} />
          ))}
          {treatment.photos.length > 4 && (
            <div style={{ width: 64, height: 64, borderRadius: 8, background: 'var(--surface-high)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'var(--text-secondary)', flexShrink: 0 }}>
              +{treatment.photos.length - 4}
            </div>
          )}
        </div>
      )}
      {treatment.specialNote && <div style={{ fontSize: 12, color: 'var(--text-tertiary)', marginTop: 8, fontStyle: 'italic' }}>메모: {treatment.specialNote}</div>}
    </div>
  );
}
